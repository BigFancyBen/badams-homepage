 ...
 */
export function exportLocationsToCSV(locations: { name: string; lat: number; lon: number }[]): string {
  if (locations.length === 0) return '';
  
  // CSV header
  const header = 'name,lat,lon';
  
  // CSV rows - escape names that contain commas by wrapping in quotes
  const rows = locations.map(loc => {
    const name = loc.name.includes(',') ? `"${loc.name}"` : loc.name;
    return `${name},${loc.lat},${loc.lon}`;
  });
...
/**
 * Import locations from CSV format
 * Expected CSV format: name,lat,lon (with or without header)
 */
export function importLocationsFromCSV(csvText: string): { id: string; name: string; lat: number; lon: number }[] {
  if (!csvText.trim()) return [];
  
  const lines = csvText.trim().split('\n');
  const locations: { id: string; name: string; lat: number; lon: number }[] = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (i === 0 && line.toLowerCase().includes('name') && line.toLowerCase().includes('lat') && line.toLowerCase().includes('lon')) {
      continue;
    }
    ...
    if (isNaN(lat) || isNaN(lon) || !isValidCoordinate(lat, lon)) {
      console.warn(`Skipping invalid coordinates on line ${i + 1}: "${line}"`);
      continue;
    }
